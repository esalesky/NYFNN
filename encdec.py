...
from conditional_gru import ConditionalGRUAttn
...
class CondGruDecoder(nn.Module):

    def __init__(self, enc_size, vocab_size, embed_size, hidden_size, bidirectional_enc=False):
        super(CondGruDecoder, self).__init__()
        self.vocab_size = vocab_size  #target vocab size
        self.hidden_size = hidden_size
    ...
    def forward(self, init_hidden, encoder_outputs, tgt, generate=False):
        ...
        if self.bidirectional_enc:
            self.hidden = torch.cat((init_hidden[0], init_hidden[1]), 1)
            # self.hidden = Variable(torch.zeros(1, batch_size, self.hidden_size))  #init to correct size
        else:
        ...
        tgt_len = tgt.shape[1]
        tgt = tgt.transpose(0, 1)
        attn_scores = self.rnn.calc_attn_scores(encoder_outputs)
        for i in range(tgt_len):
            decoder_outputs, _, decoder_hidden = self.__forward_one_word(decoder_input, encoder_outputs, attn_scores)
            # todo: potentially make teacher forcing optional/stochastic
            decoder_input = tgt[i]
            outputs.append(decoder_outputs.squeeze(1))
        return outputs
    ...
    def generate(self, init_hidden, encoder_outputs, max_gen_length, beam_size=1):
        ...
        for i in range(max_gen_length):
            ...
            for path in all_paths:
                decoder_input, decoder_contexts, hidden = beam.get_decoder_params(path)
                decoder_outputs, attn_weights, hidden = self.__forward_one_word(
                    decoder_input, encoder_outputs, attn_scores, hidden)
                # Add the potential next steps for the current beam path
                beam.add_paths(path, decoder_outputs, decoder_contexts, hidden, attn=attn_weights)
            beam.prune()
...
class AttnDecoder(nn.Module):
    ...
    def forward(self, init_hidden, encoder_outputs, tgt, generate=False):
        ...
        for i in range(tgt_len):
            decoder_outputs, decoder_contexts, attn_weights, _ = self.__forward_one_word(
        ...
        for i in range(max_gen_length):
            ...
            for path in all_paths:
                ...
                decoder_outputs, decoder_contexts, attn_weights, hidden = self.__forward_one_word(
                    decoder_input, decoder_contexts, encoder_outputs, attn_scores, hidden)
                # Add the potential next steps for the current beam path
                beam.add_paths(path, decoder_outputs, decoder_contexts, hidden, attn=attn_weights)
            beam.prune()
    ...
    def __forward_one_word(self, tgt_word, prev_context, encoder_outputs, attn_scores,
        ...
        if hidden is None:
            self.hidden = hidden
        return output, context, attn_weights, hidden
...
class EncDec(nn.Module):
    ...
    def generate(self, src, max_length, beam_size):
        self.encoder.hidden = None  # self.encoder.init_hidden(batch_size)
        encoder_outputs = self.encoder(src)
        outputs, words, attn = self.decoder.generate(
            self.encoder.hidden, encoder_outputs, max_gen_length=max_length,
            beam_size=beam_size)
