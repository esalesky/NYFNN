...
class AttnDecoder(nn.Module):
    ...
    def generate(self, init_hidden, encoder_outputs, max_gen_length, beam_size=1):
        ...
        attn_scores = self.attn.calc_attn_scores(encoder_outputs)
        #beam = Beam(beam_size)
        #beam.add_path(decoder_input, decoder_contexts, attn_scores)
        #pdb.set_trace()
        for _ in range(max_gen_length):
            decoder_outputs, decoder_contexts, attn_weights = \
                self.__forward_one_word(decoder_input, decoder_contexts,
                                        encoder_outputs, attn_scores)
            # Select the top 1 index from the scores
            top_nll, top_idx = decoder_outputs.data.topk(1)
            # Covnert the top choice into a new variable
            # Results in a new decoder input of size (batch_size, 1)
            word_idx = top_idx.cpu().numpy()
            decoder_input = Variable(torch.LongTensor(torch.from_numpy(word_idx))).squeeze(2)
            decoder_input = decoder_input.cuda() if use_cuda else decoder_input

            # Append the scores and generated words
            outputs.append(decoder_outputs.squeeze(1))
            words.append(word_idx[0][0][0])
            if word_idx == EOS:
                break
